...
public class PropActor : MonoBehaviour
{
    ...
    FloatAnim _fleeAnim = new FloatAnim(EaseType.InQuad, LoopType.PingPong, 0.25f);
    FloatRange _moveAngleRange = new FloatRange(-25f, 25f);
    ...
    void Update()
    {
        ...
        if(!_isFleeing && dst <= _disturbDst)
        {
            ...
            Emote emote = Instantiate(_disturbedEmotePrefab,
                                      transform.position + transform.right * (0.75f * dir) + Vector3.up * 0.5f,
                                      transform.rotation);
    ...
    {
        _fleeAnim.Step(dt);
        transform.rotation = Quaternion.Euler(0f, 0f, Mathf.Lerp(_moveAngleRange.Min, _moveAngleRange.Max, _fleeAnim.lerp));
