...
/// <summary>
/// Extends how ScriptableEnum object references are displayed in the inspector
/// Shows you all values under the object reference
/// Also provides a button to create a new ScriptableEnum if property is null.
/// </summary>
[CustomPropertyDrawer(typeof(ScriptableEnum), true)]
[CanEditMultipleObjects]
public class ExtendedScriptableEnumDrawer : ScriptableEnumDrawer 
{
	public override float GetPropertyHeight (SerializedProperty property, GUIContent label)
	{
		...
		if(property.isExpanded) {
			var data = property.objectReferenceValue as ScriptableEnum;
	...
	public override void OnGUI (Rect position, SerializedProperty property, GUIContent label) 
	{
		...
		ScriptableEnum propertySO = null;
		if(!property.hasMultipleDifferentValues && property.serializedObject.targetObject != null && property.serializedObject.targetObject is ScriptableEnum) {
			propertySO = (ScriptableEnum)property.serializedObject.targetObject;
		}
		...
		if(property.propertyType == SerializedPropertyType.ObjectReference && property.objectReferenceValue != null) {
			var data = (ScriptableEnum)property.objectReferenceValue;
			
			if(property.isExpanded) {
				// Draw a background that shows us clearly which fields are part of the ScriptableEnum
	...
	public static T _GUILayout<T> (string label, T objectReferenceValue, ref bool isExpanded) where T : ScriptableEnum {
		return _GUILayout<T>(new GUIContent(label), objectReferenceValue, ref isExpanded);
	}
	
	public static T _GUILayout<T> (GUIContent label, T objectReferenceValue, ref bool isExpanded) where T : ScriptableEnum {
		...
		if(objectReferenceValue != null) {
			...
			if(isExpanded) {
				DrawScriptableEnumChildFields(objectReferenceValue);
			}
		} else {
	...
	static void DrawScriptableEnumChildFields<T> (T objectReferenceValue) where T : ScriptableEnum {
		// Draw a background that shows us clearly which fields are part of the ScriptableEnum
		EditorGUI.indentLevel++;
	...
	public static T DrawScriptableEnumField<T> (GUIContent label, T objectReferenceValue, ref bool isExpanded) where T : ScriptableEnum {
		Rect position = EditorGUILayout.BeginVertical();
	...
	// Creates a new ScriptableEnum via the default Save File panel
	static ScriptableEnum CreateAssetWithSavePrompt (Type type, string path) {
		path = EditorUtility.SaveFilePanelInProject("Save ScriptableEnum", type.Name+".asset", "asset", "Enter a file name for the ScriptableEnum.", path);
		if (path == "") return null;
		...
		return asset as ScriptableEnum;
	}
